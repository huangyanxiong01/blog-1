...
从进程调用 syscall (假设是 read()) 对一个文件发出 I/O 操作，到文件系统之间的经过 (先不考虑 Page Cache Layer)：首先打开文件，打开文件时会初始化 `files_struct` =>  根据 `files_struct` 的 `fd` 找到对应的 file => 根据 file 的 `f_dentry` 找到对应的 dentry => 根据 dentry 的 `d_inode` 找到对应的 inode => 根据 syscall 的类型和 inode 的 `i_fop` 找到对应的操作函数接口 => 根据 inode 的 `i_sb` 找到对应的 superblock => 根据 superblock 找到对应的文件系统类型 (superblock 可以找到对应的 `file_system_type`)，确定文件系统类型 => VFS 将控制权交给该文件系统，并传递要执行的函数接口 => 文件系统判断要操作的文件类型 (f/c/b/s)，并调用自身实现的函数接口

当然文件系统还会继续把该 I/O 请求往下层传递，参考[分层模型](https://raw.githubusercontent.com/hsxhr-10/picture/master/%E5%9D%97%E8%AE%BE%E5%A4%87%E5%88%86%E5%B1%82%E5%9B%BE2.jpg)。
