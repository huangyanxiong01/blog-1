...
第二种情况则是发送方的 FIN 包在传输的途中 (即接收方还没收到这个 FIN 包) 时，接收方也发出了 FIN 包，并且发送方先收到接收方的 FIN 包，再收到响应的 ACK 包。

![]()

第三种情况则是发送方的 FIN 包在传输的途中 (即接收方还没收到这个 FIN 包) 时，接收方也发出了 FIN 包，并且发送方先收到响应的 ACK 包，再收到接收方的 FIN 包。
