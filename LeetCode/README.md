...
- 代码实现描述
  - 核心思想：分治
  - 特点：选中间点比较
  - 划分的实现 = 基数选择 (可引入随机选择) + 计数器初始化 + 大循环条件 while (i != j) + j 从后往前移动，找比基准数小的数，找到就交换 arr[i] 和 arr[j]，i 后移一位 + i 从前往后移动，找比基准数大的数，找到就交换 arr[i] 和 arr[j]，j 前移一位 + 最后返回基准数位置 i 或 j
...
- 代码实现描述
  ...
  - 难点：划分的实现
  - 实现标签：挖坑填数
  - 划分点的实现 = 基数选择 (可引入随机选择) + 计数器初始化 + 大循环条件 while (i != j) + j 从后往前移动，找比基准数小的数，找到就交换 arr[i] 和 arr[j]，i 后移一位 + i 从前往后移动，找比基准数大的数，找到就交换 arr[i] 和 arr[j]，j 前移一位 + 最后返回基准数位置 i 或 j
- [代码实现](https://github.com/hsxhr-10/blog/blob/master/LeetCode/quick_sort.py)
- 时间复杂度
  - 平均
  - 最差
